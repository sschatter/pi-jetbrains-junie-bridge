...
// Stable Junie CLI refuses EAP licences on chat. /auth/test and QuotaAPI
// without this header pick JUNP (USD, EAP-style) over AIP/AIPU (CREDITS),
// so the status line would show a different bucket than chat consumes.
export function grazieUserHeaders(auth, extra) {
  return {
    Authorization: auth,
    "X-Accept-EAP-License": "false",
    ...extra,
...
async function grazieQuotaPost(auth, path) {
  const upstream = await proxyFetch(`${GRAZIE_AUTH_BASE}${path}`, {
    method: "POST",
    headers: grazieUserHeaders(auth, { "Content-Type": "application/json" }),
    body: "{}",
  ...
  try {
    ...
    const [test, quota, refill] = await Promise.all([
      proxyFetch(`${GRAZIE_AUTH_BASE}/auth/test`, {
        headers: grazieUserHeaders(auth),
        signal: AbortSignal.timeout(BALANCE_TIMEOUT_MS),
      }),
