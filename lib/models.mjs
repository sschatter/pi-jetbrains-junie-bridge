...
// Anthropic keeps thinking and effort as separate knobs:
//   thinking = disabled | enabled+budget | adaptive
//   output_config.effort = low|medium|high|xhigh|max (whole-response spend)
// These Junie Claude models are adaptive-only (`forceAdaptiveThinking`).
// Pi has one slider, so we map it to *effort* and leave thinking adaptive.
// Pi hides `xhigh`/`max` unless they appear in thinkingLevelMap — without
// that it clamps them to `high`.
// Fable 5 cannot disable thinking (off → 400), so `off` is null.
// xhigh: Fable 5, Sonnet 5, Opus 5, Opus 4.8, Opus 4.7.
// max: those plus Sonnet 4.6 and Opus 4.6.
const CLAUDE_EFFORT_XHIGH_MAX = { xhigh: "xhigh", max: "max" };
const CLAUDE_EFFORT_MAX_ONLY = { max: "max" };

export const CLAUDE_THINKING_LEVEL_MAP = {
  "claude-fable-5":    { off: null, ...CLAUDE_EFFORT_XHIGH_MAX },
  "claude-sonnet-5":   CLAUDE_EFFORT_XHIGH_MAX,
  "claude-opus-5":     CLAUDE_EFFORT_XHIGH_MAX,
  "claude-opus-4-8":   CLAUDE_EFFORT_XHIGH_MAX,
  "claude-opus-4-7":   CLAUDE_EFFORT_XHIGH_MAX,
  "claude-opus-4-6":   CLAUDE_EFFORT_MAX_ONLY,
  "claude-sonnet-4-6": CLAUDE_EFFORT_MAX_ONLY,
};
  ...
  return KNOWN_GRAZIE_MODELS
    ...
    .map((id) => {
      ...
      if (type === "claude") {
        ...
        if (CLAUDE_THINKING_LEVEL_MAP[id]) {
          model.thinkingLevelMap = CLAUDE_THINKING_LEVEL_MAP[id];
        }
      } else if (type === "gemini") {
